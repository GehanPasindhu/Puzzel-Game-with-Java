 ...
 * @author pasin
 */
public class BoardGUI7 implements ActionListener {
    ...
    public void initGUI() {
        fr = new JFrame("My Puzzle Game | Level 5");
    ...
    public void displayWinMsg() {
        try {
            ...
            if (resultSet.next()) {
                ...
                ResultSet rs = (ResultSet) dbConnection.search("select * from stats where user='" + id + "' and level='5'");
                if (rs.next()) {
                    dbConnection.iud("UPDATE `stats` SET `score` = '" + countscore + "' WHERE `level` = '5' and `user`='" + id + "'");
                    ...
                    dbConnection.iud("insert into stats (user,level,score) values('" + id + "','5','" + countscore + "')");
                    JFrame frame = new JFrame("Game Win");
