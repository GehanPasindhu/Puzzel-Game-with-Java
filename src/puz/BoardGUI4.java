 ...
 * @author pasin
 */
public class BoardGUI4 implements ActionListener {
    ...
    public void initGUI() {
        fr = new JFrame("My Puzzle Game | Level 2");
    ...
    public void displayWinMsg() {
        try {
            ...
            if (resultSet.next()) {
                ...
                ResultSet rs = (ResultSet) dbConnection.search("select * from stats where user='" + id + "' and level='2'");
                if (rs.next()) {
                    dbConnection.iud("UPDATE `stats` SET `score` = '" + countscore + "' WHERE `level` = '2' and `user`='" + id + "'");
                    ...
                    dbConnection.iud("insert into stats (user,level,score) values('" + id + "','2','" + countscore + "')");
                    JFrame frame = new JFrame("Game Win");
