 ...
 * @author pasin
 */
public class BoardGUI8 implements ActionListener {
    ...
    public void initGUI() {
        fr = new JFrame("My Puzzle Game | Level 6");
    ...
    public void displayWinMsg() {
        try {
            ...
            if (resultSet.next()) {
                ...
                ResultSet rs = (ResultSet) dbConnection.search("select * from stats where user='" + id + "' and level='6'");
                if (rs.next()) {
                    dbConnection.iud("UPDATE `stats` SET `score` = '" + countscore + "' WHERE `level` = '6' and `user`='" + id + "'");
                    ...
                    dbConnection.iud("insert into stats (user,level,score) values('" + id + "','6','" + countscore + "')");
                    JFrame frame = new JFrame("Game Win");
